...
static bool chacha20_poly1305_init_wrapper(CF_AEAD_CTX *ctx) {
    return ll_CHACHA20_POLY13051305_Init((const ll_CHACHA20_POLY1305_CTX *)ctx->aead_ctx, ctx->key, ctx->key_len, ctx->iv, ctx->aad, ctx->aad_len, ctx->operation);
}
...
static bool xchacha20_poly1305_init_wrapper(CF_AEAD_CTX *ctx) {
    return ll_XCHACHA20_POLY1305_Init((const ll_CHACHA20_POLY1305_CTX *)ctx->aead_ctx, ctx->key, ctx->key_len, ctx->iv, ctx->aad, ctx->aad_len, ctx->operation);
}
