...
// Algorithm selection
//
CF_API const CF_MD *CF_MDByFlag(uint32_t algo_flag);

//
// Hash initialization / cleanup
//
CF_API CF_STATUS CF_HashInit(CF_HASH_CTX *ctx, const CF_MD *md, const CF_HASH_OPTS *opts);
CF_API CF_HASH_CTX* CF_HashInitAlloc(const CF_MD *md, const CF_HASH_OPTS *opts, CF_STATUS *status);

CF_API CF_STATUS CF_HashUpdate(CF_HASH_CTX *ctx, const uint8_t *data, size_t data_len);
CF_API CF_STATUS CF_HashFinal(CF_HASH_CTX *ctx, uint8_t *digest, size_t digest_len);

// Frees internal buffers of a pre-allocated CF_HASH_CTX
CF_API CF_STATUS CF_HashReset(CF_HASH_CTX *ctx);

// Frees internal buffers + heap-allocated CF_HASH_CTX
CF_API CF_STATUS CF_HashFree(CF_HASH_CTX **p_ctx);

//
// One-shot hash convenience
//
CF_API CF_STATUS CF_ComputeHash(
    const CF_MD        *md,
    const uint8_t      *data,
    size_t              data_len,
    uint8_t            *digest,
...
CF_API CF_STATUS CF_ComputeHashFixed(
    const CF_MD  *md,
    const uint8_t *data,
    size_t         data_len,
    uint8_t       *digest
...
//
CF_API CF_STATUS CF_CloneHashCtx(CF_HASH_CTX *dst, const CF_HASH_CTX *src);
CF_API CF_HASH_CTX *CF_CloneHashCtxAlloc(const CF_HASH_CTX *src, CF_STATUS *status);

CF_API size_t CF_HashGetDigestSize(const CF_HASH_CTX *ctx);  // fixed-output hashes
CF_API size_t CF_HashGetBlockSize(const CF_HASH_CTX *ctx);
CF_API const char* CF_HashGetName(const CF_MD *md);

//
// options initialization / cleanup
//
CF_API CF_STATUS CF_HashOptsInit(
    CF_HASH_OPTS *opts,
...
CF_API CF_HASH_OPTS* CF_HashOptsInitAlloc(
    const uint8_t *N, size_t N_len,
    const uint8_t *S, size_t S_len,
    size_t out_len,
    CF_STATUS *status
...
CF_API CF_STATUS CF_HashOptsReset(CF_HASH_OPTS *opts);
CF_API CF_STATUS CF_HashOptsFree(CF_HASH_OPTS **p_opts);

CF_API CF_STATUS CF_CloneHashOpts(CF_HASH_OPTS *dst, const CF_HASH_OPTS *src);
CF_API CF_HASH_OPTS *CF_CloneHashOptsAlloc(const CF_HASH_OPTS *src, CF_STATUS *status);
